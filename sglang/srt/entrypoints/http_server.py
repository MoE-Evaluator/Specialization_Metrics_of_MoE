...
async def dump_expert_distribution_record_async():
    ...
    if result.data is not None:
        ...
        serializable_data = tensor_to_serializable(result.data)
        return Response(
        ...
        return Response(
            content="Dump expert distribution record completed (data saved to file).\n",
            status_code=200,
        )


@app.api_route("/get_expert_distribution_status", methods=["GET", "POST"])
async def get_expert_distribution_status_async():
    """Get current expert distribution recording status."""
    result = await _global_state.tokenizer_manager.get_expert_distribution_status()
    if result.data is not None:
        import json
        import torch
        ...
        return Response(
            content="Unable to get expert distribution status.\n",
            status_code=500,
        )
